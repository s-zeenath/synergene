...
export default function OldPredictionsPage() {
  ...
  if (!isLoaded || !isSignedIn) {
    return (
      <main className="relative min-h-screen bg-blue-100" style={!isLight ? {} : {}}>
  ...
  return (
    <main className="relative min-h-screen w-full bg-blue-100" style={colors.pageBg}>
                ...
                <thead className={`${colors.thead} sticky top-0 z-10 text-xs uppercase tracking-wide`}>
                  ...
                  <tbody>
                    {rows.map((p: PredRow, idx: number) => {
                      ...
                      const score =
                        typeof p.score === "number" ? Number(p.score.toFixed(1)) : (p.score as any);
                      const scoreCls =
                        score >= 80
                          ? "text-green-500"
                          : score >= 60
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-3 text-center align-middle">
                            <span className={`inline-flex items-center justify-center rounded-full px-3 py-1.5 text-xs font-semibold shadow-sm ${scoreCls}`}>
                              {score}%
                            </span>
                          </td>
                          <td className="px-5 py-3 text-center align-middle">
